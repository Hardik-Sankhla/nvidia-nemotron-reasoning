...
- Other approaches of your choice
...
The only requirement is that the final submission produces a compatible LoRA adapter for the Nemotron-3-Nano-30B base model (adapter must include an `adapter_config.json`).
...
Submissions are evaluated based on accuracy in solving the provided tasks. The [NVIDIA Nemotron-3-Nano-30B](https://www.kaggle.com/models/metric/nemotron-3-nano-30b-a3b-bf16) model is loaded with your LoRA adapter using the vLLM inference engine. For each test case, the model is prompted to generate a response and is instructed to place its final answer within a LaTeX boxed command (e.g., $\boxed{\text{answer}}$). The metric extracts the final answer from the generated text, prioritizing content within the boxed format while falling back to other heuristic patterns or the last numeric value found.

A prediction is graded as correct if it matches the ground truth either exactly as a string or within a relative numerical tolerance (if numeric $10^{-2}$). The final score is the proportion of correctly answered questions.
...
- **June 8, 2026** — Entry Deadline (must accept competition rules before this date)
- **June 8, 2026** — Team Merger Deadline (last day to join or merge teams)
...
Awarded to the team with the highest leaderboard score as of the Midpoint Cut-off Date (April 9, 2026). Methodology submissions cut-off: April 16, 2026. Winners will be announced during Cloud NEXT (April 22–24, 2026).
...
The Open Contribution Awards recognize techniques that meaningfully advance reasoning performance using Nemotron models. Three awards will be granted:

- Best Data / Synthetic Data Method — 1 DGX Spark
- Best RL Method — 1 DGX Spark
- Best Fine-tuning Method — 1 DGX Spark

Participants must submit their entry for these awards through the [designated form](https://docs.google.com/forms/d/1O9XN6v0fCDkBVChbbcorBmT95rhca6_ELz8RwwJnx9c/preview?resourcekey=0-I6m8wE4Cap8m5NuOBLk_Cg) linking to their notebook and clearly identifying the category being entered. Only submissions ranking within the top 10% of the final leaderboard will be considered for Open Contribution Awards.

## Compute (NVIDIA Blackwell on Google Cloud)

We're excited to partner with Google Cloud to offer G4 VMs powered by NVIDIA RTX PRO 6000 Blackwell Server Edition GPUs as the compute backbone for this challenge. G4 VMs are well suited for both fine-tuning and high‑throughput inference with Nemotron models, enabling rapid iteration on prompts, data pipelines, and tuning strategies while evaluating performance on real benchmarks.https://docs.cloud.google.com/compute/docs/accelerator-optimized-machines#g4-series

Learn more about the underlying Google Cloud G4 VM and its capabilities on the Google Cloud documentation.
